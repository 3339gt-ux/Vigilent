...
1. **System Readiness & Diagnostics Panel**:
   ...
   - Displays environment availability status (presence/absence of Supabase keys, application mode, local storage DB size, and expected storage bucket).
...
2. **In-App Password Recovery Completion**:
   - Implemented an elegant, native in-app password update flow in `src/app/login/page.tsx` that triggers automatically when the user arrives via a Supabase password reset link (detecting `#type=recovery` or `access_token` in the URL).
...
5. **Safe Database Error Masking**:
   - Hardened error handling by mapping all PostgREST/PostgreSQL raw database errors, syntax faults, and RLS violations to clean, user-friendly messages.
...
- [ ] **Password Recovery Smoke**: Append `#type=recovery&access_token=test` to `/login`. Verify the Set New Password form renders.
...
- Verify the following table structures exist: `saved_reports`, `vigilen_assets`, `asset_check_assignments`.
