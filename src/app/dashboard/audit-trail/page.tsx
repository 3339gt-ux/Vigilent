...
export default function AuditTrailPage() {
  ...
  const performExportCSV = () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `assurecore-audit-trail-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const performExportJSON = () => {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `assurecore-audit-trail-${new Date().toISOString().split('T')[0]}.json`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
