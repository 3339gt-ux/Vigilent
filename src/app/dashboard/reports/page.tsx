...
export default function ReportsPage() {
  ...
  }, [startDate, endDate, selectedOwner, selectedCategory, selectedStatus, selectedRisk, includeInactive, includeArchived, includeDeactivated, comparePeriod]);
  ...
  const handleExportCSV = (reportName: string, headers: string[], rows: string[][]) => {
    setConfirmRequest({
      ...
      onConfirm: () => {
        try {
          const csvContent = [
            [`LUMÉN Compliance Report - ${reportName}`],
            [`Workspace: ${organization?.name || 'LUMÉN Demo'}`],
          ...
          link.href = url;
          link.setAttribute('download', `lumen-report-${reportName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handlePrintReport = (reportName: string, selectorId: string) => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>LUMÉN Report - ${escapeHtml(reportName)}</title>
          <style>
        ...
        <body>
          <h1>LUMÉN Readiness and Evidence Report</h1>
          <h2>Report: ${escapeHtml(reportName)} | Workspace: ${escapeHtml(organization?.name || 'LUMÉN Workspace')}</h2>
          ...
          <div class="disclaimer">
            Reports reflect the records currently held in LUMÉN and depend on the completeness and accuracy of the underlying data.
          </div>
        </body>
      </html>
    `);
          ...
          <AdvancedControlsPanel isOpen={showFilters} onClose={() => setShowFilters(false)}>
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1">Requirement / Category</label>
                  <select
                ...
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1">Requirement / Owner</label>
                  <select
                    value={selectedOwner}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1">Requirement Status</label>
                  <select
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-border/40">
                <button
                  ...
                  className="px-3 py-1.5 bg-muted hover:bg-muted/80 text-foreground font-bold rounded-lg cursor-pointer text-xs"
                >
                  Reset Filters
                </button>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-6">
                ...
                {filteredPrebuilt.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-extrabold uppercase text-muted-foreground tracking-widest">LUMÉN Prebuilt Reports</h3>
        ...
        {activeTab === 'builder' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              {/* Configuration panel */}
              {(interfaceDetailLevel !== 'focused' || showFilters) && (
                <div className="bg-card border border-border p-5 rounded-2xl space-y-4 shadow-sm text-xs lg:col-span-1">
              ...
              <div className={`bg-card border border-border p-5 rounded-2xl space-y-4 shadow-sm min-h-[300px] ${(interfaceDetailLevel !== 'focused' || showFilters) ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-card border border-border rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
            ...
            <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-xl space-y-2.5 text-xs text-muted-foreground leading-relaxed text-left">
              ...
              <p>
                Automated report deliveries and email scheduling are currently not configured in the LUMÉN platform database.
              </p>
              <p>
          ...
          <div className="bg-card border border-border rounded-2xl max-w-4xl w-full h-[85vh] flex flex-col p-6 space-y-4 shadow-xl relative animate-scaleIn">
            <div className="flex justify-between items-start gap-4 shrink-0">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Calculation logic, exclusions, boundaries, and source modules for all LUMÉN metrics.
                </p>
              </div>
              <button
                onClick={() => {
